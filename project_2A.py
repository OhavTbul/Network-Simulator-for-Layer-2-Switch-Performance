...
class Host(Base):
    ...
    def recieve_L2Message(self, msg: L2Message,link=None ):
        if msg.dest_mac_address != self.host_mac_address: #not all masseges are directed to this host - flooding
            print(f"{RED}the rcieved is {self.host_mac_address} but arrived {msg.dest_mac_address}")
            msg.alive = False #TODO try
            return
        print(f"{GREEN}host mac: {self.host_mac_address} recieved massage ") if FLAG_HOST else None
    ...
    def recieve_L2Message(self, msg: L2Message, link_id):
        ...
        for nic_id in self.ports:
            if nic_id != None and nic_id != link_id:
                ...
                print(f"{PINK}performing flooding, old msg ID {msg.ID})\nnew mgs ID:{flood_msg.ID}")
